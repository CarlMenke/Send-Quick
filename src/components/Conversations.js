...
const mapDispatchToProps = (dispatch) => {
  return {
    fetchSendFriendRequest: (senderId, recieverName) =>
      dispatch(loadSendFriendRequest(senderId, recieverName)),
    fetchUpdateUserSocket: (name, socket) =>
      dispatch(loadUpdateSocketId(name, socket)),
    fetchSetRecieverSocket: (name) => dispatch(loadSocketFromName(name)),
    fetchUserDetails: (userId) => dispatch(loadUserDetails(userId)),
    fetchFriendRequestResponse: (userId, friendId, choice) =>
      dispatch(loadFriendRequestResponse(userId, friendId, choice)),
    fetchOpenChat: (reciever, sender) =>
      dispatch(loadOpenChat(reciever, sender)),
    fetchUpdateUserSocket: (name, socket) =>
      dispatch(loadUpdateSocketId(name, socket)),
...
const Conversations = (props) => {
  ...
  const sendFriendRequest = async (e) => {
    e.preventDefault();
    await props.fetchSendFriendRequest(
      loggedUser.id,
      currentFriendReqRecipient
    );
      ...
      <div className="conversations">
        <div className={`conversations-container-${!chatBox}`}>
          ...
          <div className="conversations-list">
            {loggedUser.friend && recentMessagesArray
              ? loggedUser.friend.map((user, index) => {
                  return (
                    <div
                      onClick={() => {
                        handleOpenChatBox(user, loggedUser);
                      }}
                      className="conversation-card"
                      key={index}
                    >
                      <div className="conversation-card-name">{user.name}</div>
                      <div className="conversation-card-message">
                        {recentMessagesArray[index]}
                      </div>
                    </div>
                  );
                })
              : null}
            {loggedUser.friendrequestrecieved
              ? loggedUser.friendrequestrecieved.map((user, index) => {
                  return (
            ...
            >
              <input
                className="add-input"
                onChange={(e) => {
                  setCurrentFriendReqRecipient(e.target.value);
                }}
